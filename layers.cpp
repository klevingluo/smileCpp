...
class layers {
  ...
  private:
    ...
    const vector<int> head = {1,2,3,4,5,6,7,8,9,0,18,17,16,15,14,13,12,11,10,83,84,85,86,87,88,89,90};
  ...
  public:
    layers(Mat face, vector<Point> landmarks) {
      ...
      for (int i=1; i<9; i++) {
        landmarks_[82+i] = Point(cos(M_PI/9 * i)*rad + center.x,-0.85 * sin(M_PI/9 * i)*rad + center.y);
      }
      face_ = face.clone();
    };
    Mat getLightness() {
    ...
    Mat getShapes() {
      ...
      fillConvexPoly(mask, &getPoly(nose)[0], nose.size(), 0,8,0);
      ...
      int wt = face_.cols / 20;
      int ht = face_.rows / 20;
      if(wt % 2 != 1) {
        wt += 1;
      }
      if(ht % 2 != 1) {
      ...
      cv::GaussianBlur(mask, mask, Size(wt*2+1,ht*2+1), 0,0);
